		...
		<table id="treeTable" class="table table-striped table-bordered table-condensed">
			...
			<tbody><c:forEach items="${list}" var="menu">
				...
				<tr id="${menu.id}" pId="${menu.parent.id ne '1'?menu.parent.id:'0'}">
					...
					<td nowrap><i class="icon-${not empty menu.icon?menu.icon:' hide'}"></i><a href="${ctx}/sys/menu/form?id=${menu.id}"> <c:out value="${menu.name }"></c:out></a></td>
					<td title="${menu.href}">${fns:abbr(menu.href,30)}</td>
					<td >
						<shiro:hasPermission name="sys:menu:edit">
							<input type="hidden" name="ids" value="${menu.id}"/>
							<input name="sorts" type="text" value="${menu.sort}" style="width:50px;height:30px;" class="form-control text-center">
