...
demo.Util = {
    ...
    registerImageByWh: function (url,width,height, svg) {
        var image = new Image();
        image.src = url;
        image.width = width;
        image.height = height;
        var views = arguments;
        image.onload = function () {
            twaver.Util.registerImage(demo.Util.getImageName(url), image, width, height, svg === true);
            image.onload = null;
            for (var i = 1; i < views.length; i++) {
                var view = views[i];
                if (view.invalidateElementUIs) {
...
//        demo.Util.addButton(toolbar, 'Pan', 'pan', function () { network.setPanInteractions(); });
    ...
    addInput: function (div, id, value, callback) {
    ...
    addLabel: function (div,id,value){
    	 var label = document.createElement('label');
         label.id = id;
