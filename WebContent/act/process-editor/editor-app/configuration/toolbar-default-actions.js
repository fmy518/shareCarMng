				...
				$http(
						...
						.success(
								function(data, status, headers, config) {
									...
									KISBPM.eventBus
											.dispatch(
													KISBPM.eventBus.EVENT_TYPE_MODEL_SAVED,
													saveEvent);
						...
						.error(
								function(data, status, headers, config) {
									$scope.error = {};
									console
											.log('Something went wrong when updating the process model:'
													+ JSON.stringify(data));
									$scope.status.loading = false;
								});
			};
