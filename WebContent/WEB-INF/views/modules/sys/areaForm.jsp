...
<body>
	...
	<form:form id="inputForm" modelAttribute="area" action="${ctx}/sys/area/save" method="post" class="form-horizontal" role="form" >
		<form:hidden path="id"/>
		<sys:message content="${message}"/>
		<div class="form-group">
			<label class="control-label col-sm-2">上级区域:</label>
			<div class="col-sm-10">
				<sys:treeselect id="area" name="parent.id" value="${area.parent.id}" labelName="parent.name" labelValue="${area.parent.name}"
					title="区域" url="/sys/area/treeData" extId="${area.id}" cssClass="form-control input-medium" allowClear="true"/>
			</div>
		</div>
		<div class="form-group">
		...
		<div class="form-group">
			<label class="control-label  col-sm-2">区域编码:</label>
			<div class="col-sm-10">
				<form:input path="code" htmlEscape="false" maxlength="50"  Class="form-control input-medium"/>
			</div>
		</div>
		<div class="form-group">
			<label class="control-label  col-sm-2">区域类型:</label>
			<div class="col-sm-10">
		...
		<div class="form-group">
		  ...
		  <div class="col-sm-10">
			<shiro:hasPermission name="sys:area:edit"><input id="btnSubmit" class="btn btn-default btn-primary" type="submit" value="保 存"/>&nbsp;</shiro:hasPermission>
